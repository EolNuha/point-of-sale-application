...
def signin():
    ...
    if check_password_hash(user.password, password):
        token = jwt.encode(
            {"public_id": user.public_id, "exp": datetime.utcnow() + timedelta(days=1)},
...
def users():
    ...
    return jsonify(
        getPaginatedDict(getUsersList(paginated_items.items), paginated_items)
    )
...
def deleteUser(id):
    ...
    if user.sales or user.purchases:
        return (
            "This user can not be deleted because it has sales or purchases connected to it!",
            500,
        )
...
def currentUser():
    ...
    return jsonify(
        getUserDict(User.query.filter_by(public_id=data["public_id"]).first_or_404())
    )


@auth_rest.route("get-all-users")
class GetAllUsers(Resource):
    def get(self):
        users = User.query.all()
        return getUsersList(users)
